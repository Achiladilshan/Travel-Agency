            ...
            connection.query(EmailQuery, [UserID], (err, rows) => {
                ...
                connection.query(
                    ...
                    (err, result) => {
                        ...
                        // Check if total payments for the trip are equal to trip price
                        ...
                        connection.query(TotalPaymentsQuery, [TripID], (err, rows) => {
                            ...
                            connection.query(TripPriceQuery, [TripID], (err, rows) => {
                                ...
                                if (TotalPayments === TripPrice) {
                                    connection.query(
                                        'UPDATE Trip SET Status = "Active" WHERE TripID = ?',
                                        [TripID],
                                        (err, result) => {
                                            if (err) {
...
const getPaymentsByTripID = (req, res) => {
    const { TripID } = req.params;

    connection.query('SELECT * FROM CustomerPayment WHERE TripID = ?', [TripID], (err, rows) => {
        if (err) {
            console.error('Error querying payment:', err);
            return res.status(500).send('Internal Server Error');
        }
