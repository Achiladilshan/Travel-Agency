...
const TripCard = (allocatedTrip) => {
    ...
    // Fetch allocated trip on component mount
    useEffect(() => {
        const fetchTripData = async () => {
            try {
                try {
                    const res = await instance.get(`/trip/`);
                    const SelectedTrip = res.data.filter(selectedtrip =>
                        selectedtrip.TripID === allocatedTrip.allocatedTrip
                    );
                            ...
                            <div className={`flex flex-col w-full mb-4 p-4 bg-[#f2f2f2] rounded-lg shadow-md`}>
                                <div className="flex items-center">
                                    <div className="ml-8 mr-[60px] space-y-3 w-[50%]">
                                        <div>Trip ID: {trip[0].TripID}</div>
                                        <div>Customer ID: <b>{trip[0].CustomerID}</b></div>
                                        <div>Customer Name: <b>{trip[0].FirstName} {trip[0].LastName}</b></div>
                                        <div>Customer Email: <b>{trip[0].Email}</b> </div>
                                        <div>Customer Phone: <b>{trip[0].PhoneNumber}</b></div>
                                        <div>Customer Country: <b>{trip[0].Country}</b></div>
                                        <div>Start Date: <b>{formatDate(trip[0].StartDate)}</b></div>
                                        <div>End Date: <b>{formatDate(trip[0].EndDate)}</b></div>
                                        <div>Adults Count: <b>{trip[0].AdultsCount}</b></div>
                                        <div>Children Count: <b>{trip[0].ChildrenCount}</b></div>
                                        <div>Price: <b>$ {trip[0].Price}</b></div>
                                    </div>
                                    <Divider orientation="vertical" flexItem />
                                    <div className="flex mx-8 w-[40%] justify-center">
                                        <div>Special Note: {trip[0].SpecialNotes}</div>
                                    </div>
                                    <Divider orientation="vertical" flexItem />
                                    <div className="flex mx-8 w-[20%] justify-center">
                                        <div className="flex space-x-10 items-center">
                                            <div className='inline-flex flex-col space-y-3'>
                                                <button className="bg-[#39069e] text-white px-4 py-2 rounded-xl" onClick={() => handleModalToggle(trip[0])}>More Details</button>
                                                <button className="bg-[#198061] text-white px-4 py-2 rounded-xl" onClick={() => handleNestedModalToggle(trip[0])}>View Trip</button>
                                            </div>
                                        </div>
                                    </div>
                                    <Divider orientation="vertical" flexItem />
                                    <div className="mx-9 flex justify-center">
                                        <div className="flex items-center justify-center">
                                            <button className={`px-4 py-2 rounded-2xl bg-[green] text-white`} disabled>
                                                <b>{trip[0].Status}</b>
                                            </button>
                                        </div>
                                    </div>
                ...
                {showNestedModal && (
                    <div className="modal modal-open">
                        <div className="modal-box max-w-3xl overflow-hidden flex flex-col">
                            ...
                            <div className="px-4 overflow-auto max-h-[40vh]">
